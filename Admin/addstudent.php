<?php
if(!isset ($_SESSION)){
    session_start();
}
include('./admininclude/header.php');
include('../dbConnection.php');
if(isset($_SESSION['is_admin_login'])){
    $adminEmail = $_SESSION['admLogEmail'];
}
else{
    echo "<script> location.href='http://localhost/TechLearning/index.php'; </script>";
}

$stu_email = $stu_name = $stu_class = $stu_pass = "";
if(isset($_REQUEST['stuSubmitBtn'])){
    $stu_name = $_REQUEST['stu_name'];
    $stu_email = $_REQUEST['stu_email'];
    $stu_pass = $_REQUEST['stu_pass'];
    $stu_class = $_REQUEST['stu_class'];
    // check if field is empty
    if($stu_name == "" || $stu_email == "" || $stu_pass == "" || $stu_class == ""){
        $msg = "<div class='alert alert-warning col-sm-6 ml-5 mt-2'>Fill All The Fields</div>";
    }
    else{
        $sql = "INSERT INTO student (stu_name,stu_email,stu_pass,stu_class) values ('$stu_name','$stu_email','$stu_pass','$stu_class')";

            if($conn->query($sql) == TRUE){
               $msg = "<div class='alert alert-success col-sm-6 ml-5 mt-2'> Student Added Successfully</div>";
            } else{
                $msg = "<div class='alert alert-danger col-sm-6 ml-5 mt-2'>Unable to Add Student</div>";
            }
    }
}
?>


<div class="jumbotron col-sm-6 mt-5 mx-3">
<h3 class="text-center">Add new Student</h3>
<form action="" method="POST" enctype="multipart/form-data">
<div class="form-group">
<label for="stu_name"><b>Student Name</b></label>
<input type="text" class="form-control" id="stu_name" name="stu_name">
</div>
<div class="form-group">
<label for="stu_email"><b>Student Email</b></label>
<input type="text" class="form-control" id="stu_email" name="stu_email">
</div>
<div class="form-group">
<label for="stu_pass"><b>Student Password</b></label>
<input type="text" class="form-control" id="stu_pass" name="stu_pass">
</div>
<div class="form-group">
<label for="stu_class"><b>Student Class</b></label>
<input type="text" class="form-control" id="stu_class" name="stu_class">
</div>

<div class="text-center" style="margin-top:20px;">
    <button type="submit" class="btn btn-danger" id="stuSubmitBtn" name="stuSubmitBtn" >Submit</button>
    <a href="students.php" class="btn btn-secondary">Close</a>
</div>
<?php if(isset($msg)) {echo $msg;} ?>
</form>
</div>
</div><!--Div row close from header -->
</div><!--Div container-fluid close from header -->
<?php
include('./admininclude/footer.php');
?>
